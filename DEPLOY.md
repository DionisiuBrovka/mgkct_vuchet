...
## Обновление после перехода на английские имена

Техническое имя пакета и Docker-образа — `mgkct_teaching_hours`.
Применяется новая миграция `1788048000000_english_names.js`: коллекции теперь
называются `teaching_report_entries` и `substitutions`, поля часов — как в README.
Миграция изменяет имена существующих коллекций и полей, сохраняя их ID и данные.
Используются штатные [операции с коллекциями](https://pocketbase.io/docs/js-collections/)
в [транзакционной миграции](https://pocketbase.io/docs/js-migrations/).

**Старый клиент несовместим с новой схемой.** До обновления остановите запись и
сохраните резервную копию всего `pb_data` при остановленном PocketBase. Затем:

1. Перед первой локальной сборкой после переименования выполните `flutter clean`
   и `flutter pub get`, чтобы удалить кэш со старым именем пакета. Соберите новый
   клиент; скопируйте на сервер весь каталог миграций. Docker собирает клиент
   в отдельном окружении и не использует локальный кэш Flutter.
2. При остановленном сервере выполните `./pocketbase migrate up --automigrate=false`
   из каталога сервера и замените содержимое `pb_public` новой веб-сборкой.
3. Запустите сервер, обновите страницы клиентов и проверьте вход, часы и статусы.

Для Docker `entrypoint.sh` применяет миграции до запуска API; образ уже содержит
новый клиент. Сохраните прежний Compose project name, расположение compose-файла
и существующий том: не выполняйте `down -v` и не переносите базу в новый пустой том.
Для Podman имя тома остаётся `mgkct_data`; для Compose имя тома зависит также
от имени проекта. Изменение имени образа не требует изменения тома или `/pb/pb_data`.

Для отката убедитесь, что последняя миграция — именно
`1788048000000_english_names.js`. При остановленном сервере выполните:

```bash
./pocketbase migrate down 1 --automigrate=false
```

Затем верните соответствующую старую веб-сборку.
Сначала проверяйте обновление и откат на копии базы.

**Нативные приложения:** Android/Linux application ID и Apple bundle ID также
переименованы. ОС считает сборку с новым ID отдельным приложением; это не обновление
установленной версии со старым ID. Не распространяйте её как бесшовное обновление:
для существующих установок потребуется отдельная установка и повторный вход.
Серверные данные сохраняются. Веб-версия продолжает работать по прежнему адресу.
...
# Скопируйте ВСЕ миграции (включая переименование полей и коллекций):
#   scp tools/pocketbase/pb_migrations/*.js server:/opt/mgkct/pb_migrations/
```
